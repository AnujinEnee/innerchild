...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from("articles").select("id, title, category, subcategory, author_id, reviewed_by, published_date, content, image_url, status").eq("id", id).single(),
      supabase.from("team_members").select("id, last_name, first_name").order("created_at"),
    ]).then(([articleRes, teamRes]) => {
      const members = (teamRes.data ?? []).map((m: { id: string; last_name: string; first_name: string }) => ({
        id: m.id, name: `${m.last_name} ${m.first_name}`.trim(),
      }));
      ...
      if (data) {
        setForm({
          ...
          reviewed_by_id: members.find((m: { id: string; name: string }) => m.name === data.reviewed_by)?.id ?? "",
  ...
  // Sync editor content when editor becomes ready
  useEffect(() => {
  ...
  async function save() {
    console.log("save() called, title:", form.title);
    if (!form.title.trim()) { console.log("title empty, returning"); return; }
    setSaving(true);
    const currentContent = editor ? editor.getHTML() : form.content;
    const selectedAuthor = teamMembers.find((m) => m.id === form.author_id);
    const reviewedMember = teamMembers.find((m) => m.id === form.reviewed_by_id);
    const payload = {
    ...
    console.log("Save payload:", JSON.stringify({ reviewed_by_id: form.reviewed_by_id, reviewedMember, reviewed_by: payload.reviewed_by }));
    const { error } = await createClient().from("articles").update(payload).eq("id", id);
    setSaving(false);
  ...
  if (loading) {
    return <div className="py-20 text-center"><p className={`text-sm ${dark ? "text-white/30" : "text-gray-400"}`}>Уншиж байна...</p></div>;
  }

  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className={`text-2xl font-bold ${dark ? "text-white" : "text-gray-900"}`}>Нийтлэл засах</h1>
        <button
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select value={form.reviewed_by_id} onChange={(e) => setForm({ ...form, reviewed_by_id: e.target.value })} className={`w-full rounded-xl px-4 py-2.5 text-sm outline-none ${inputCls}`}>
